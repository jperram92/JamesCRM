...
const CompanyChat = ({ companyId }) => {
  ...
  const { messages, loading, error, success } = useSelector((state) => state.companies);
  const { user } = useSelector((state) => state.auth);
  const companyMessages = messages[companyId] || [];
  ...
  useEffect(() => {
    ...
    dispatch(markCompanyMessagesAsRead(companyId));
  ...
  const handleSendMessage = (e) => {
    ...
    dispatch(createCompanyMessage({ id: companyId, content: messageContent }));
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-300px)] min-h-[500px]">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        {loading && companyMessages.length === 0 ? (
