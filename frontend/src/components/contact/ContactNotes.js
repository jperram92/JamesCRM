...
const ContactNotes = ({ contactId }) => {
  ...
  const { notes, loading, error, success } = useSelector((state) => state.contacts);
  const contactNotes = notes[contactId] || [];
  
  const [noteContent, setNoteContent] = useState('');
  
  // Load notes on component mount
  useEffect(() => {
    dispatch(getContactNotes(contactId));
  }, [dispatch, contactId]);
  ...
  const handleCreateNote = (e) => {
    ...
    dispatch(createContactNote({ id: contactId, content: noteContent }));
  };
